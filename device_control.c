...
void get_identify(cJSON *root)
{
	...
	kill_gateway();
	else if(open_wifi_file() < 0)
	{
		cJSON *user_id=  cJSON_CreateObject();
		cJSON *user_data =  cJSON_CreateObject();
		cJSON_AddStringToObject(user_id,"co","orange");
		cJSON_AddStringToObject(user_id,"api","gw_add_user");
		cJSON_AddItemToObject(user_id,"data",user_data);
		cJSON_AddStringToObject(user_data,"union_id",u_id);
		char *send_char = cJSON_PrintUnformatted(user_id);
		int my_len = strlen(send_char);
		char *my_send_char = (char *)malloc(my_len+2);
		memset(my_send_char,0,my_len+2);
		memcpy(my_send_char,send_char,my_len);
		strcat(my_send_char,"\n\0");
		while(gw_add_user_flag==0)
		{
			send(cd,my_send_char,my_len+1,0);
			usleep(200000);
		}
		cJSON_Delete(user_id);
		user_id=NULL;
		free(send_char);
		send_char=NULL;
		free(my_send_char);
